...
%Our objective is to avoid relying too heavily on (often unavailable) quantitative measurements,
%while retaining enough descriptive power to be an asset in the study of biological networks.
...
Reactions are abstracted by \emph{interactions} intended as cause-and-effect relations. Thus,
...
% Figure~\ref{fig:abstraction-mek-erk} contains a first example of \tas\ which we use to explain
% the basic features of the formalism on which our models are founded.
...
\def\mekTA{\includegraphics[scale=.098]{images/abstraction_ta_erk2}}
\newlength\mekTAheight
\setlength\mekTAheight{\heightof{\mekTA}}
\begin{figure}[!hb]
%\begin{minipage}{\textwidth}
\begin{center}
\subfloat[\label{subfig:mek-erk}]{\begin{minipage}[c][\mekTAheight]{0.14\textwidth}\begin{center}\includegraphics[scale=.098]{images/abstraction_ta_mek-erk3}\end{center}\end{minipage}}
\ \
...
\ \
\subfloat[\label{subfig:erk}]{\begin{minipage}[c][\mekTAheight]{0.12\textwidth}\begin{center}\mekTA\end{center}\end{minipage}}
\end{center}
...
In this example, MEK activity is not regulated and ERK has only two activity levels,
completely inactive or completely active.
...
active\_ERK} location) and the local clock coupled to this automaton is reset, ${\sf t} := 0$.
{\bf \protect\subref{subfig:erk}}~A \ta\ model of ERK, consisting of two locations, {\sf inactive\_ERK}
(the starting location) and {\sf active\_ERK},
and one transition between the locations. This transition will take place when it is possible to synchronize with
the corresponding action {\sf activate\_ERK!} in the MEK automaton.
The synchronization on channel {\sf activate\_ERK} represents the occurrence of the activating
interaction between MEK and ERK, and allows ERK to become completely active.}\label{fig:abstraction-mek-erk}
%\end{minipage}
\end{figure}
