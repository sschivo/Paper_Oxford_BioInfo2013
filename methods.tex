...
interactions to give a formal description of network dynamics (\citealt{animo-bibe}). An example of a small 
Timed Automata model illustrates the basic properties of \tas\~\ref{fig:abstraction-mek-erk}. 
...
Each new node in the network has to be initialized with the number of activity levels and its initial activity. 
...
table of nodes or interactions. The ANIMO user workflow is explained in Suppl. Video 1.
...
Nevertheless, the \tas\ model and the model checking process in UPPAAL are accessible when desired by the user.
...
upon an output graph from the model (Figure~\ref{fig:small-model} B,D,F).
...
we computed the {\sf Casp3\_{}data} series as $100\% - [\mbox{\sf pro-Casp3}]$.

image were taken from the ANIMO user interface.
